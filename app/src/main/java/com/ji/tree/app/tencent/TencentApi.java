package com.ji.tree.app.tencent;

import com.ji.tree.utils.InternetUtils;

public class TencentApi {
    public static SearchAppData search(String kw) {
        SearchAppData searchAppData = new SearchAppData();
        String s = InternetUtils.getString("https://sj.qq.com/myapp/searchAjax.htm?"
                + "&kw=" + kw);
        return searchAppData;
    }

    public static TopAppData softTop(int pageNo, int pageSize) {
        TopAppData topAppData = new TopAppData();
        String s = InternetUtils.getString("https://mapp.qzone.qq.com/cgi-bin/mapp/mapp_applist?apptype=soft_top&platform=touch"
                + "&pageNo=" + pageNo + "&pageSize" + pageSize);
        return topAppData;
    }
}
